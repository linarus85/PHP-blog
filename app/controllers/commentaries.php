...
$commentsForAdm = selectAllTable('comments');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['goComment'])) {
    ...
    if ($email === '' || $comment === '') {
        ...
        $user = selectTable('users', ['email' => $email]);
        ...
        $comments = selectAllTable('comments', ['page' => $page, 'status' => 1]);
    }
} else {
    // $email = '';
    // $comment = '';
...
if ($_SERVER['REQUEST_METHOD'] === 'GET' && isset($_GET['id'])) {
    $oneComment = selectTable('comments', ['id' => $_GET['id']]);
    $id =  $oneComment['id'];
